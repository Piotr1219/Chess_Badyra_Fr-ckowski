...
string Program::HumanMove(string move) {
	if (move.length() < 4)
	{
		return "";
	}
	...
	//std::cout << "wypisanie tablicy tylko po ruchu czlowiekaw nowej funkcji" << endl;
	//DrawBoard(engine);
	...
	if (engine.StaleMate())
	{
		...
		if (engine.InsufficientMaterial())
		{
			res = "1/2-1/2 {Draw by insufficient material}";
		}
		else if (engine.RepeatedMove())
		{
			res = "1/2-1/2 {Draw by repetition}";
		}
		else if (engine.FiftyMove())
		{
			res = "1/2-1/2 {Draw by fifty move rule}";
		}
		else
		{
			res = "1/2-1/2 {Stalemate}";
		}
		engine.NewGame();
		return res;
	}
	...
	return engine.ChessBoard.ToString();
}

string Program::HumanMove1(string move) {
	if (move.length() < 4)
	{
		return "";
	}
...
string Program::get_board() {
	return engine.ChessBoard.ToString();
}

int Program::set_level(int level) {
	if (level == 0) {
		engine.setGameDifficulty(Engine::Difficulty::Easy);
		return 0;
	}
		...
		engine.setGameDifficulty(Engine::Difficulty::VeryHard);
		return 0;
	}
	else {
		return 1;
