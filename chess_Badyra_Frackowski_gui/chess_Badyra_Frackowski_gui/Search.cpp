...
int Search::AlphaBeta(Board& examineBoard, char depth, int alpha, int beta, int& nodesSearched, int* nodesQuiessence, list<Position>* pvLine, bool extended, double& time1, double& time2, double& time3)
{
    ...
    if (examineBoard.WhiteCheck || examineBoard.BlackCheck || positions.size() == 0)
    {
        ...
        if (SearchForMate(examineBoard.WhoseMove, examineBoard))
        {
...
bool Search::SearchForMate(ChessPieceColor movingSide, Board& examineBoard)
{
    ...
    for (short x = 0; x < 64; x++)
    {
        ...
        for (char dst : examineBoard.Squares[x].Piece1.ValidMoves)
        {
            ...
            //We make copies of the board and move so that we can move it without effecting the parent board
            //Board board = examineBoard.FastCopy();
            Board board = Board(examineBoard);
