...
class Book
{
    ...
    list<OpeningMove> PopulateOpeningBook()
    {
        ...
        moveLine = R"(e2e4{3820} b2b3{15} c2c4{540} d2d4{2619} g1f3{851} g2g3{5})";
        char* char_arr;
        char_arr = &moveLine[0];
        char* token = strtok(char_arr, " ");
        while (token != NULL)
        {
            openingMove.Moves.Add(MoveContent(token));
            token = strtok(NULL, " ");
        }
        openingMove = OpeningMove();
        ...
        while (token != NULL)
        {
            openingMove.Moves.Add(MoveContent(token)); token = strtok(NULL, " ");
        }
        openingBook.Add(openingMove);
        openingMove = OpeningMove();
